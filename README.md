...
* *Updated* formula sheet for the 3rd exam is posted [here](https://github.com/rwb/c200/blob/main/gfiles/f3b.pdf).
...
* The formula sheet for exam #3 has been updated to include the test statistic for comparing a sample mean to a population mean and the confidence interval for comparing a sample mean to a population mean (both from Lesson 26); [link to formula sheet](https://github.com/rwb/c200/blob/main/gfiles/f3b.pdf).
* I had a question for you about practice problem 10.2c for week 15. The Z score for 120 is .667 and the area found for 0.66 was 0.2454 and the area for 0.67 was 0.2486. But the z-table in Appendix 3 only goes out to 2 digits which violates the 3 digit rounding rule. Which z score should be used? My response: I would accept either one in this case; it's a borderline situation because 0.66 corresponds to 0.2454 and 0.67 corresponds to 0.2486; since the z-table only goes out to 2 decimal places (not 3) the z-score of 0.667 actually rounds to 0.67 so 0.2486 would be the better answer but I would accept either one. I did go over the 2-digit problem on the z-table last week in class and I said I would not penalize for this ambiguity.
* If I have two z-scores that are both below the mean, how do I find the area between them since the z-table doesn't have negative z-scores? My response: It's the same as when the 2 scores are above the mean; the area between a zscore of 1 and 2 is the same as the area between a zscore of -1 and -2 (this is the symmetry property of the normal distribution; the area below the mean is a mirror image of the area above the mean).
*  I had a question about 10.2 c and 10.3 b. Both problems are asking for the percentage of scores between 2 scores. To solve 10.2c you subtract the areas 0.2486-0.1293. For question 10.3b however, you multiply 0.2357 by 2. Why are they different? My response: for problem 10.3b, one of the scores is above the mean and one is below the mean. For problem 10.2c, both scores are above the mean. To solve these problems, you have to keep in mind that the area between a zscore of 1 and 2 is the same as the area between a zscore of -1 and -2 (note: I'm just using the numbers 1 and 2 as examples here; you could also say the area between a z-score of 0 and 0.625 is the same as the area between 0 and -0.625 which is what is in problem 10.3b).
* I have a question about problem 10.3c: since the number is over the mean why do we add 0.5 instead of subtracting 0.5? My response: that problem is asking you the area between a z-score of -infinity and 3.125 (which is the same thing as asking what fraction of students are below 110). So, you take the 50 percent of the observations below the mean and add the 49.91% of the observations between the mean and a zscore of 3.125 (or a raw score of 110) to get a total of 99.91% of the observations below 110.
* I have been practicing single-sample z-tests and wondering if the tests will be two-tailed, one-tailed, or a mixture of both? My response: as I mentioned in class both last week and on Tuesday, the z-tests for this exam will be non-directional (two-tailed) tests.
* The example given in topic 51 states "that the area between z=0 and z=+1.96 corresponds to 47.5% of the area under the standard normal curve", was this done by looking in appendix 3 for the area that when added to itself will equal to 95%? And is this the method for looking for the critical value for all two-tailed tests?" My response: Yes, when it's a 2-tailed test at the 95% confidence level, then 0.025 area will be in each tail.
